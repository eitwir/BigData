    ...
    with connection.cursor() as cursor:
        for movie_id, title, genres in movies_data:
            ...
            for genre in list_genres:
                with open(sql_script_path) as file:
                    script = file.read()
                    cursor.execute(script.format(int(movie_id), connection.escape(title[:-7]), int(year), connection.escape(genre)))
                connection.commit()
...
if __name__ == "__main__":
    create_connection_to_server_and_create_db('localhost',
                                              'root',
                                              '1234567890',
                                              r'D:\DE\Homework\HW_3_SQL_python\files\sql\CREATE_DATABASE_movies_db.sql'
                                              )
    connection = get_connection_to_db('localhost',
                                      'root',
                                      '1234567890',
                                      'movies_db'
                                      )
    ...
    fill_movies_table(connection, movies, r'D:\DE\Homework\HW_3_SQL_python\files\sql\FILL_MOVIES_TABLE.sql')
    fill_rating_table(connection, rating, r'D:\DE\Homework\HW_3_SQL_python\files\sql\FILL_RATING_TABLE.sql')
    create_table(connection, r'D:\DE\Homework\HW_3_SQL_python\files\sql\CREATE_TABLE_avg_rating.sql')
    create_table(connection, r'D:\DE\Homework\HW_3_SQL_python\files\sql\CREATE_TABLE_result_movies_table.sql')
